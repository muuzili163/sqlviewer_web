// Configuration
const config = {
    baseUrl: 'https://sql-out.sdcreditech.com',
...
// HTTP utilities
const http = {
    async get(url, csrftoken, sessionid) {
    ...
    async post(url, data, csrftoken, sessionid) {
...
const api = {
    async login(username, password) {
        try {
            ...
            if (result.status === 0) {
                ...
                config.sessionid = storage.get('sessionid') || '';
                config.csrftoken = storage.get('csrftoken') || config.csrftoken;
    ...
    async queryServers() {
        try {
            const csrftoken = storage.get('csrftoken') || config.csrftoken;
            const sessionid = storage.get('sessionid') || '';
            const result = await http.get(
                `${config.baseUrl}/group/user_all_instances/?tag_codes%5B%5D=can_read`,
                csrftoken,
                sessionid
            );
    ...
    async queryDatabases(instanceName) {
        try {
            ...
            const sessionid = storage.get('sessionid') || '';
            const result = await http.get(
                `${config.baseUrl}/instance/instance_resource/?instance_name=${instanceName}&resource_type=database`,
                csrftoken,
                sessionid
            );
            return result.data || [];
    ...
    async queryTables(instanceName, dbName) {
        try {
            ...
            const sessionid = storage.get('sessionid') || '';
            const result = await http.get(
                `${config.baseUrl}/instance/instance_resource/?instance_name=${instanceName}&db_name=${dbName}&resource_type=table`,
                csrftoken,
                sessionid
            );
            return result.data || [];
    ...
    async showTableStructure(dbName, tableName) {
        try {
            ...
            const sessionid = storage.get('sessionid') || '';
            const result = await http.post(
                `${config.baseUrl}/instance/describetable/`,
                {
        ...
        try {
            ...
            const result = await http.post(
                `${config.baseUrl}/query/`,
                {
                    ...
                    tb_name: tableName,
                    sql_content: sql,
                    limit_num: config.pageSize.toString()
                },
                csrftoken,
                sessionid
            );
...
class UIController {
    ...
    bindEvents() {
        ...
        document.getElementById('pageSizeSelect').addEventListener('change', (e) => {
            ...
            storage.set('pageSize', e.target.value);
        });
    ...
    populateServers(servers) {
        ...
        const savedInstance = storage.get('instance_name');
    ...
    async handleServerChange(instanceName) {
        ...
        storage.set('instance_name', instanceName);
